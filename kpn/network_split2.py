...
class KPN(nn.Module):
    def __init__(self, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
        ...
        self.conv2 = Basic(128, 128, channel_att=False, spatial_att=False)  # 64*64
        self.conv3 = Basic(512, 256, channel_att=False, spatial_att=False)  # 64*64
        self.split_block2=B.Split_block5(256,256,3,activation='relu')
        ...
        self.desplit_0=B.DeSplitting()
        self.conv7_1 = Basic(64, 64, channel_att=False, spatial_att=False) 
        self.conv8 = Basic(256 + 64, 256, channel_att=channel_att, spatial_att=spatial_att)
        self.conv8_1 = Basic(64, 64, channel_att=False, spatial_att=False) 
        self.desplit_1=B.DeSplitting()
        self.conv9 = Basic(128 , 64, channel_att=channel_att, spatial_att=spatial_att)
    ...
    def forward(self, data_with_est, x):
        ...
        conv2 = self.split_block1(conv1) # 128*128*128
        conv2=self.conv2(conv2)
        conv2 = torch.cat([conv2, x], dim=1)
        conv3 = self.conv3(self.split_block2(conv2)) # 256*64*64
        ...
        conv7=self.desplit_0(conv7)
        conv7=self.conv7_1(conv7)
        conv8 = self.conv8(torch.cat([conv2, conv7], dim=1))  # 128*128*128
        conv8=self.desplit_1(conv8)
        conv8=self.conv8_1(conv8)
        conv9 = self.conv9(torch.cat([conv1, conv8], dim=1))  # 64*256*256
