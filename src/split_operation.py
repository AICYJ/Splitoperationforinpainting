...
from .model_split_operation import EdgeModel, InpaintingModel
...
class Split_Operation():
    ...
    def load(self):
        if self.config.MODEL == 1:
            ...
            if self.config.MODEL_LOAD == None:
                pass
            else:
                self.inpaint_model.load(self.config.MODEL_LOAD)
                print('resume train: {}'.format(self.config.MODEL_LOAD))
    ...
    def eval(self):
        ...
        with torch.no_grad():
            ...
            message='iteration:{} ave_psnr:{}  ave_ssim:{} ave_l1:{}  ave_lpips:{}'.format(
                iteration,
                np.average(psnr_all),
                np.average(ssim_all),
                np.average(l1_list),
            ...
            log_path=os.path.join(self.config.PATH, 'training.txt')
            f = open(log_path, 'a')
            f.write(message+'\n')
            f.close()
        ...
        with torch.no_grad():
            for items in test_loader:
                ...
                if len(ssim_list) % 1 == 0:
                    ...
                    kpn_utils.save_sample_png_test(sample_folder=self.config.TEST_SAMPLE_SAVE, sample_name='{}_'.format(len(ssim_list)),
                                              img_list=img_list,
